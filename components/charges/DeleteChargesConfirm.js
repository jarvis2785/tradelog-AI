...
export default function DeleteChargesConfirm({ row, onCancel, onConfirm, deleting }) {
  return (
    <AnimatePresence>
      <motion.div
        key="backdrop"
          ...
          <div className="flex items-start gap-3 mb-5">
            ...
            <div>
              <h3 className="text-h3 text-text-primary">
                Delete charges for {row?.date ? toDDMMYYYY(row.date) : "this date"}?
              </h3>
              <p className="text-body text-text-secondary mt-1">
                Net P&L will revert to Overall P&L.
              </p>
            </div>
          </div>
