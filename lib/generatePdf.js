...
export function generateReportPdf(
  ...
  // "overall" is a live, ever-growing period — its saved report.total_trades/net_pnl etc.
  // are only a point-in-time snapshot and go stale once new trades are logged, so re-derive
  // them from the (already unfiltered) trades/charges instead. Weekly/monthly periods are
  // closed, so their snapshot stays accurate and is left untouched.
  const isOverall = reportType === "overall";
  const liveStats = isOverall ? computeLiveTradeStats(trades) : null;
  ...
  statLine("Total Trades", totalTrades);
  statLine("Win Rate", `${winRate.toFixed(1)}%`);
  statLine("Overall P&L", formatCurrency(chargesSummary.totalOverallPnl));
  statLine("Net P&L", formatCurrency(netPnl));
  ...
  if (reportType !== "weekly") {
    ...
    if (!operatingExpenses || !operatingExpenses.hasData) {
      ...
      const economic = computeEconomicPnl(netPnl, operatingExpenses.total);
  ...
  if (reportType === "overall") {
    ...
    if (!capital || !capital.hasStartingCapital) {
      paragraph("Set up your trading account in the Accounts section to see capital tracking.");
    } else {
      statLine("Starting Capital", formatCurrency(capital.startingCapital));
      statLine("Total Deposited", formatCurrency(capital.totalDeposited));
      statLine("Total Trading P&L", formatCurrency(capital.totalTradingPnl));
      statLine("Total Withdrawn", `-${formatCurrency(capital.totalWithdrawn)}`);
      statLine("Current Account Value", formatCurrency(capital.currentValue));
      statLine(
        "Trading Return",
        `${capital.tradingReturn >= 0 ? "+" : ""}${capital.tradingReturn.toFixed(1)}%`
      );
      statLine(
        "Account Growth",
        `${capital.accountGrowth >= 0 ? "+" : ""}${capital.accountGrowth.toFixed(1)}%`
      );
      statLine("Peak Capital", formatCurrency(capital.peakCapital));
      statLine(
        "Max Drawdown from Peak",
        `${formatCurrency(capital.drawdownAmount)} (${capital.drawdownPercent.toFixed(1)}%)`
      );
    }
    y += 8;
  }
  ...
  statLine("Rule Violations", ruleViolations);
  y += 8;
